...
MER.ElvUIX = tonumber(GetAddOnMetadata("ElvUI_AzUI", "X-ElvVersion"))
...
MER.Logo = [[Interface\AddOns\ElvUI_AzUI\Core\Media\Textures\AzUI_logo_Small.tga]]
MER.LogoSmall = [[Interface\AddOns\ElvUI_AzUI\Core\Media\Textures\AzUI_logo_Small.tga]]
...
E.PopupDialogs.AzUI_BUTTON_FIX_RELOAD = {
	text = format(
		"%s\n%s\n\n|cffaaaaaa%s|r",
...
_G.BINDING_HEADER_MER = "|cffff7d0aAzUI|r"
for i = 1, 5 do
...
function MER:AddMoverCategories()
	tinsert(E.ConfigModeLayouts, #(E.ConfigModeLayouts) + 1, "AzUI")
	E.ConfigModeLocalizedStrings["AzUI"] = format("|cffff7d0a%s |r", "AzUI")
end
	...
	if E.global.mui.core.cvarAlert then
		self:RegisterEvent("CVAR_UPDATE", function(_, cvar, value)
			if cvar == "ActionButtonUseKeyDown" and MER.UseKeyDown ~= (value == "1") then
				E:StaticPopup_Show("AzUI_BUTTON_FIX_RELOAD")
			end
		end)
	end
end

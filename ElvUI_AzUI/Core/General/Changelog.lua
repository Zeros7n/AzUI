...
local ChangeLogData = {
	"Changes:",
		"• Updated MerathilisUI code to work with the latest version of Wow Classic Anniversary",
		-- "• ''",
...
function MER:CreateChangelog()
	local frame = CreateFrame("Frame", "AzUIChangeLog", E.UIParent, 'BackdropTemplate')
	frame:Point("CENTER")
	frame:Size(600, 430)
...
function MER:CountDown()
	...
	if self.time == 0 then
		AzUIChangeLog.countdown:SetText("")
		AzUIChangeLog.close:Enable()
		self:CancelAllTimers()
	else
		AzUIChangeLog.countdown:SetText(format("(%s)", self.time))
	end
end

function MER:ToggleChangeLog()
	if not AzUIChangeLog then
	...
	fadeInfo.endAlpha = 1
	E:UIFrameFade(AzUIChangeLog, fadeInfo)

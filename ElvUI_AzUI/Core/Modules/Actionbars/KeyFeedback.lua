...
function keyFeedback:PLAYER_LOGIN()
	...
	E:CreateMover(self, "SpellFeedback",  L["SpellFeedback"], nil, nil, nil, "ALL,ACTIONBARS,AzUI", function() return E.db.mui.actionbars.keyfeedback end, "mui,modules,actionbars")
...
function keyFeedback:RefreshSettings()
	...
	if db.enableCastLine then
        ...
        pool:ReleaseAll()
        if pool and pool.EnumerateInactive then -- 25.7.2 ADDED CHECK HERE
...
local bg = S:CreateBDFrame(mirror)
    ...
    if a2 then 
        if a2.SetAlphaFrom then
            a2:SetAlphaFrom(1)
            a2:SetAlphaTo(0)
        elseif a2.SetFromAlpha then
            a2:SetFromAlpha(1)
            a2:SetToAlpha(0)
        else
        end
        ...
        print("AzUI KeyFeedback: Warning: Alpha animation object 'a2' for glow effect could not be created.")
    end

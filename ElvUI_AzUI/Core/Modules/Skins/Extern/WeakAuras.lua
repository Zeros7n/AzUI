...
local wa_regionPrototype = WeakAuras and WeakAuras.regionPrototype

if wa_regionPrototype and type(wa_regionPrototype) == "table" and type(wa_regionPrototype.create) == "function" then
    -- If the WeakAuras regionPrototype and its 'create' method are found and are of the expected types,
    -- then proceed with the hook.
    module:SecureHook(wa_regionPrototype, "create", OnPrototypeCreate)
else
end

if WeakAuras and WeakAuras.regionPrototype and WeakAuras.regionPrototype.modifyFinish then

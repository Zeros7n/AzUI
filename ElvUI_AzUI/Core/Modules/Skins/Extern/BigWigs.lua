...
local function modifyStyle(frame)
	...
	local barColor = frame:Get("bigwigs:Azui:barcolor")

	if db.colorOverride then
		local statusBarTexture = frame.candyBarBar:GetStatusBarTexture()

		if not barColor then
			frame:Set("bigwigs:Azui:barcolor", { statusBarTexture:GetVertexColor() })
		end
	...
	local spark = frame:Get("bigwigs:Azui:spark")
...
local function applyStyle(frame)
	...
	frame:Set("bigwigs:Azui:originalheight", height)
	...
	spark:SetBlendMode("ADD")
	spark:Show()
	frame:Set("bigwigs:Azui:spark", spark)
	...
	barBackdrop:Show()
	frame:Set("bigwigs:Azui:barbackdrop", barBackdrop)

	frame:Set("bigwigs:Azui:barbackgroundisshown", frame.candyBarBackground:IsShown())
	...
	if tex then
		...
		frame:Set("bigwigs:Azui:iconpoints", getPoints(frame.candyBarIconFrame))
		...
		frame:Set("bigwigs:Azui:tex", tex)
		...
		iconBackdrop:Show()
		frame:Set("bigwigs:Azui:iconbackdrop", iconBackdrop)
	end

	frame:Set("bigwigs:Azui:durationpoints", getPoints(frame.candyBarDuration))
	...
	frame:Set("bigwigs:Azui:labelpoints", getPoints(frame.candyBarLabel))
...
local function barStopped(frame)
	local durationPoints = frame:Get("bigwigs:Azui:durationpoints")
	if durationPoints then
		applyPoints(frame.candyBarDuration, durationPoints)
		frame:Set("bigwigs:Azui:durationpoints", nil)
	end

	local labelPoints = frame:Get("bigwigs:Azui:labelpoints")
	if labelPoints then
		applyPoints(frame.candyBarLabel, labelPoints)
		frame:Set("bigwigs:Azui:labelpoints", nil)
	end

	local iconBackdrop = frame:Get("bigwigs:Azui:iconbackdrop")
	if iconBackdrop then
		pool:Release(iconBackdrop)
		iconBackdrop:Hide()
		frame:Set("bigwigs:Azui:iconbackdrop", nil)
	end

	local iconPoints = frame:Get("bigwigs:Azui:iconpoints")
	if iconPoints then
		applyPoints(frame.candyBarIconFrame, iconPoints)
		frame:Set("bigwigs:Azui:iconpoints", nil)
	end

	local tex = frame:Get("bigwigs:Azui:tex")
	if tex then
		frame:SetIcon(tex)
		frame:Set("bigwigs:Azui:tex", nil)
	end

	local barBackgroundIsShown = frame:Get("bigwigs:Azui:barbackgroundisshown")
	if barBackgroundIsShown then
		frame.candyBarBackground:SetShown(barBackgroundIsShown)
		frame:Set("bigwigs:Azui:barbackgroundisshown", nil)
	end

	local barBackdrop = frame:Get("bigwigs:Azui:barbackdrop")
	if barBackdrop then
		pool:Release(barBackdrop)
		barBackdrop:Hide()
		frame:Set("bigwigs:Azui:barbackdrop", nil)
	end
	...
	if spark then
		spark:Hide()
		pool:Release(spark)
		frame:Set("bigwigs:Azui:spark", nil)
	end

	local barColor = frame:Get("bigwigs:Azui:barcolor")
	if barColor then
		frame.candyBarBar:GetStatusBarTexture():SetVertexColor(unpack(barColor))
		frame:Set("bigwigs:Azui:barcolor", nil)
	end

	E:SetSmoothing(frame.candyBarBar, false)

	local height = frame:Get("bigwigs:Azui:originalheight")
	if height then
...
function module:BigWigs_QueueTimer()
	...
	if _G.BigWigsLoader then
		_G.BigWigsLoader.RegisterMessage("AzUI", "BigWigs_FrameCreated", function(_, frame, name)

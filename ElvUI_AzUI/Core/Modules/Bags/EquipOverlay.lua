...
-- DELETE OR COMMENT OUT THIS LINE: local EquipmentManager_UnpackLocation = EquipmentManager_UnpackLocation
...
local quickFormat = {
    ...
    [3] = function(font, map)
        font:SetFormattedText("|cff70C0F5%s %s %s|r", Utf8Sub(map[1], 1, 4), Utf8Sub(map[2], 1, 4),
            Utf8Sub(map[3], 1, 4))
    end
}
        ...
        local SetInfoTable = C_EquipmentSet_GetItemLocations(equipmentSetID)
            for _, location in pairs(SetInfoTable) do
                    if type(location) == "number" and (location < -1 or location > 1) then
                        -- Check if UnpackLocation exists before calling it
                            if type(C_EquipmentSet.UnpackLocation) == "function" then
                            player, bank, bags, _, slot, bag = C_EquipmentSet.UnpackLocation(location)
...
local function UpdateContainerFrame(frame, bag, slot)
    ...
    if (frame.equipmentinfo) then
        ...
        if module.equipmentMap[key] then
            quickFormat[#module.equipmentMap[key] < 4 and #module.equipmentMap[key] or 3](frame.equipmentinfo,

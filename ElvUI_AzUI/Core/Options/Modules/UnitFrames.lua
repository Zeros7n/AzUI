...
--[[
25.8.2 FIX:
AzUI was calling E:StaticPopup_Show("VERSION_MISMATCH") without defining the dialog.
This defines a minimal safe popup so ElvUI's StaticPopups.lua no longer iterates a nil dialog.
]]
E.PopupDialogs = E.PopupDialogs or {}
if type(E.PopupDialogs["VERSION_MISMATCH"]) ~= "table" then
    E.PopupDialogs["VERSION_MISMATCH"] = {
        text = (L and L["Invalid Model ID entered. Please enter a numeric ID."]) or
            "Invalid Model ID entered. Please enter a numeric ID.",
        button1 = ACCEPT,
        OnAccept = E.noop or function()
        end,
        timeout = 0,
        whileDead = 1,
        hideOnEscape = true
...
local DeathIndicatorImages = {
    MATERIAL = [[|TInterface\AddOns\ElvUI_AzUI\Core\Media\Textures\materialDead:14|t]],
    SKULL = [[|TInterface\LootFrame\LootPanel-Icon:14|t]],
    SKULL1 = [[|TInterface\AddOns\ElvUI_AzUI\Core\Media\Textures\SKULL:16|t]],
    SKULL2 = [[|TInterface\AddOns\ElvUI_AzUI\Core\Media\Textures\SKULL1:16|t]],
    SKULL3 = [[|TInterface\AddOns\ElvUI_AzUI\Core\Media\Textures\SKULL2:16|t]],
    SKULL4 = [[|TInterface\AddOns\ElvUI_AzUI\Core\Media\Textures\SKULL3:16|t]],
    CUSTOM = L["CUSTOM"]
}

local OfflineIndicatorImages = {
    MATERIAL = [[|TInterface\AddOns\ElvUI_AzUI\Core\Media\Textures\materialDC:14|t]],
                ...
                power = {
                    ...
                    args = {
                        ...
                        customModel = {
                            ...
                            -- 25.8.2 FIX: Show the (now-defined) popup and cleanly return false.
                            ...
                            set = function(_, value)
                                E.db.mui.unitframes.power.model = tonumber(value)
                                -- 25.8.2 FIX: move reload popup into the setter where it belongs.
                                E:StaticPopup_Show("CONFIG_RL")
                            end
                        },
                        texture = {
                        ...
                        elvui = {
                            ...
                            args = {
                                feature = {
                                    ...
                                    type = "description",
                                    name = format("%s\n%s",
                                        format(L["The absorb style %s and %s is highly recommended with %s tweaks."],
                                            F.CreateColorString(L["Overflow"], E.db.general.valuecolor),
                                            F.CreateColorString(L["Auto Height"], E.db.general.valuecolor), L["AzUI"]),
                                        L["Here are some buttons for helping you change the setting of all absorb bars by one-click."])
                                },
                                setAllAbsorbStyleToOverflow = {
                                    order = 2,
                                    type = "execute",
                                    name = format(L["Set All Absorb Style to %s"],
                                        F.CreateColorString(L["Overflow"], E.db.general.valuecolor)),
                                    func = function(info)
                                ...
                                changeColor = {
                                    order = 4,
                                    type = "execute",
                                    name = format(L["%s style absorb color"], L["AzUI"]),

...
-- Check if the Mixin table exists AND if its SetClubInfo method is a function
if CommunitiesListEntryMixin and type(CommunitiesListEntryMixin) == "table" and type(CommunitiesListEntryMixin.SetClubInfo) == "function" then
    -- If the target exists and is valid, then perform the hook
    hooksecurefunc(CommunitiesListEntryMixin, "SetClubInfo", function(self, clubInfo, isInvitation, isTicket)
        -- This is where the original skinning logic (if any) would go.
        -- Assuming the hook's purpose is just to *skin* it,
        -- the original function call is handled by hooksecurefunc itself.

        -- You can add debug prints here if you ever need to confirm it's running on Retail:
        -- print("AzUI: CommunitiesListEntryMixin:SetClubInfo hooked successfully.")
    end)
else
end
